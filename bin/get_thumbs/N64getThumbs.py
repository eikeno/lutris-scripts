...
def get_covers():
	...
	cur = conn.cursor()
	cur.execute("SELECT slug, configpath FROM games WHERE platform='Nintendo N64'")
	rows = cur.fetchall()
	for row in rows:
		sys.stderr.write('-----------------------------\n')
		slug=str(row[0]).replace('-!-', '')
		...
		name = os.path.basename(main_file).replace('.v64', '')
		name = os.path.basename(name).replace('.z64', '')
		name = os.path.basename(name).replace('.zip', '')
