...
def get_covers():
	...
	cur = conn.cursor()
	cur.execute("SELECT slug, configpath FROM games WHERE platform='Atari 800'")
	rows = cur.fetchall()
	for row in rows:
		slug=str(row[0])
		...
		name = os.path.basename(main_file).replace('.zip', '').replace('.atr', '').replace('.atx', '').replace('bas', '').replace('bin', '').replace('cas', '').replace('com', '').replace('rom', '').replace('xex', '')
