...
def get_covers():
	...
	cur = conn.cursor()
	cur.execute("SELECT slug, configpath FROM games WHERE platform='Playstation 2';")
	rows = cur.fetchall()
	for row in rows:
		slug=str(row[0])
		...
		main_file = y['game']['main_file']
		sys.stderr.write('args: ' + str(main_file) + '\n')
		
		#name = os.path.basename(main_file).replace('.iso', '').replace('.chd', '').replace('.cue', '')
		name = os.path.basename(main_file)
		sys.stderr.write('name: ' + name + '\n')
		
		canon = re.sub(r'\(.*?\)', '', name)
		canon = re.sub(r'\[.*?\]', '', canon)
		#canon = canon.replace(', The', '').replace('.chd', '')
		canon = re.sub("\S{9}.chd", "", canon) # strip SERIAL code
		canon = canon.strip()
