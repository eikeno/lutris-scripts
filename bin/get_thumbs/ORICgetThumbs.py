...
destpattern_image = "%s/.cache/lutris/coverart/oric/" % _home
...
def get_covers():
	...
	cur = conn.cursor()
	cur.execute("SELECT slug, configpath FROM games WHERE slug like '%-oric'")
	rows = cur.fetchall()
	for row in rows:
		slug=str(row[0])
		...
		main_file=str(main_file).split("-cart")[-1]
