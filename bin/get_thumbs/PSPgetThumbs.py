...
def get_covers():
	...
	cur = conn.cursor()
	cur.execute("SELECT slug, configpath FROM games WHERE platform='Sony PSP'")
	rows = cur.fetchall()
	for row in rows:
		slug=str(row[0])
		...
		name = os.path.basename(main_file).replace('.iso', '').replace('.chd', '')
		sys.stderr.write('name: ' + name + '\n')
		
		canon = re.sub(r'\(.*?\)', '', name)
		canon = re.sub(r'\[.*?\]', '', canon)
		canon = canon.replace(', The', '')
		canon = re.sub("\S{8}", "", canon) # strip SERIAL code
		canon = canon.strip()
