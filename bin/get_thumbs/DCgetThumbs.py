...
def get_covers():
	...
	cur = conn.cursor()
	cur.execute("SELECT slug, configpath FROM games WHERE platform='Sega Dreamcast'")
	rows = cur.fetchall()
	for row in rows:
		sys.stderr.write('-----------------------------\n')
		...
		main_file = y['game']['iso']
		sys.stderr.write('iso: ' + main_file + '\n')
		
		name = os.path.basename(main_file).replace('.gdi', '')
		name = os.path.basename(name).replace('.cdi', '')
