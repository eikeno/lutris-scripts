...
def get_covers():
	...
	cur = conn.cursor()
	cur.execute("SELECT slug, configpath FROM games WHERE platform='ZX Spectrum'")
	rows = cur.fetchall()
	for row in rows:
		slug=str(row[0])
		...
		name = os.path.basename(main_file).replace('.zip', '').replace('.z80', '').replace('.tap', '').replace('.dsk', '').replace('.tzx', '')
