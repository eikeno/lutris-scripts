...
GAME_NAME="$(basename "$@" ".iso")"
GAME_NAME=$(echo "$GAME_NAME" | sed 's/\!//g' | sed 's/\[\]//g' | sed 's/.chd//' )

echo "GAME_NAME = $GAME_NAME"
SLUG="$(string_to_slug.sh "$GAME_NAME")""-psp"
...
INSERT INTO games (name, slug, platform, runner, installed, installed_at, configpath, playtime, hidden) VALUES ( "$GAME_NAME", "$SLUG", "Sony PSP", "ppsspp", 1, $DATE_S, "$SLUG-$DATE_S", 0.0, 0);
EOT
