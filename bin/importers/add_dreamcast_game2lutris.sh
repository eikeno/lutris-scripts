...
GAME_NAME="$(basename "$@" ".gdi")"
GAME_NAME="$(echo "$GAME_NAME" | sed 's/.cdi//g')"

echo "GAME_NAME = $GAME_NAME"
SLUG="$(string_to_slug.sh "$GAME_NAME")""-dc"
...
INSERT INTO games (name, slug, platform, runner, installed, installed_at, configpath, playtime, hidden) VALUES ( "$GAME_NAME", "$SLUG", "Sega Dreamcast", "flycast", 1, $DATE_S, "$SLUG-$DATE_S", 0.0, 0);
EOT
