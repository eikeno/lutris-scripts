...
GAME_NAME="$(basename "$@" ".tap")"
GAME_NAME=$(echo "$GAME_NAME" | sed 's/\!//g' | sed 's/\[\]//g' | sed 's/.zip//')

echo "GAME_NAME = $GAME_NAME"
SLUG="$(string_to_slug.sh "$GAME_NAME")""-gmaster"
...
cat << EOT
game:
  args: gmaster  -skip_gameinfo -video opengl -window -cart "$@"
  exe: mame
linux: {}
system:
  locale: ''
...
INSERT INTO games (name, slug, platform, runner, installed, installed_at, configpath, playtime, hidden) VALUES ( "$GAME_NAME", "$SLUG", "Linux", "linux", 1, $DATE_S, "$SLUG-$DATE_S", 0.0, 0);
EOT
