...
GAME_NAME="$(basename "$@" ".bin")"
GAME_NAME="$(basename "$GAME_NAME" ".zip")"
GAME_NAME="$(basename "$GAME_NAME" ".z80")"
GAME_NAME="$(basename "$GAME_NAME" ".tap")"
GAME_NAME="$(basename "$GAME_NAME" ".dsk")"
GAME_NAME="$(basename "$GAME_NAME" ".tzx")"

GAME_NAME=$(echo "$GAME_NAME" | sed 's/\!//g' | sed 's/\[\]//g')

echo "GAME_NAME = $GAME_NAME"
SLUG="$(string_to_slug.sh "$GAME_NAME")""-zxs"
...
INSERT INTO games (name, slug, platform, runner, installed, installed_at, configpath, playtime, hidden) VALUES ( "$GAME_NAME", "$SLUG", "ZX Spectrum", "libretro", 1, $DATE_S, "$SLUG-$DATE_S", 0.0, 0);
EOT
