...
GAME_NAME="$(basename "$GAME_NAME" ".32x")"
GAME_NAME="$(basename "$GAME_NAME" ".md")"
GAME_NAME=$(echo "$GAME_NAME" | sed 's/\!//g' | sed 's/\[\]//g')

echo "GAME_NAME = $GAME_NAME"
SLUG="$(string_to_slug.sh "$GAME_NAME")""-mark3"
...
INSERT INTO games (name, slug, platform, runner, installed, installed_at, configpath, playtime, hidden) VALUES ( "$GAME_NAME", "$SLUG", "Sega Mastersystem", "libretro", 1, $DATE_S, "$SLUG-$DATE_S", 0.0, 0);
EOT
