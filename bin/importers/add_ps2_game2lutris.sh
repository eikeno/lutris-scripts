...
GAME_NAME="$(basename "$GAME_NAME" ".chd")"

GAME_NAME=$(echo "$GAME_NAME" | sed 's/\!//g' | sed 's/\[\]//g' | sed 's/ [[:alnum:]]\{9\}.chd//')

echo "GAME_NAME = $GAME_NAME"
SLUG="$(string_to_slug.sh "$GAME_NAME")""-ps2"
...
INSERT INTO games (name, slug, platform, runner, installed, installed_at, configpath, playtime, hidden) VALUES ( "$GAME_NAME", "$SLUG", "Playstation 2", "pcsx2", 1, $DATE_S, "$SLUG-$DATE_S", 0.0, 0);
EOT
